...
import { NavItem, ThemeSwitcher } from "../atoms";

const navItems:{id:number, text:string, href:string}[] =[
    {
        id:1,
        text:"Accueil",
        href:"/home/"
...
export function Navbar() {
    ...
    return (
        {/*
            <header>
            <nav className="fixed inset-x-0 z-20 w-full border-b border-gray-100 bg-white/80 backdrop-blur dark:border-gray-700/30 dark:bg-gray-900/80">
                <Container>
                    <div className="relative flex flex-wrap items-center justify-between gap-6 lg:gap-0 lg:py-4">
                        <div className="relative z-20 flex w-full justify-between md:px-0 lg:w-max">
                            ...
                            <button onClick={
                                toggleNavbar
                            } aria-label="humburger" className="relative -mr-6 p-6 lg:hidden">
                                ...
                                <div aria-hidden="true" className={`mx-auto h-0.5 w-5 rounded bg-sky-900 transition duration-300 dark:bg-gray-300 ${navOpen ? "translate-y-1.5 rotate-45" : ""}`} />
                                <div aria-hidden="true" className={`mx-auto mt-2 h-0.5 w-5 rounded bg-sky-900 transition duration-300 dark:bg-gray-300 ${navOpen ? "-translate-y-1 -rotate-45" : ""}`} />
                            </button>
                        </div>
                        <div onClick={closeNav} aria-hidden="true" className="fixed inset-0 z-10 h-screen w-screen origin-bottom scale-y-0 bg-white/70 backdrop-blur-2xl transition duration-500 dark:bg-gray-900/70 lg:hidden" />
                        ...
                        <div onClick={closeNav} aria-hidden="true" className={`fixed inset-0 z-10 h-screen w-screen origin-bottom bg-white/70 backdrop-blur-2xl transition duration-500 dark:bg-gray-900/70 lg:hidden ${navOpen ? "origin-top scale-y-100 lg:hidden lg:scale-y-0" : " scale-y-0"}`} />
                        <div id="navlinks" className={`
                                absolute top-full left-0 z-20 w-full flex
                                origin-top-right translate-y-1 scale-90 flex-col flex-wrap justify-end gap-6 rounded-3xl 
                                border border-gray-100 bg-white p-8 shadow-2xl shadow-gray-600/10 transition-all 
                                duration-300 dark:border-gray-700 dark:bg-gray-800 dark:shadow-none lg:relative lg:flex lg:w-auto  
                                lg:flex-row lg:items-center lg:gap-0 lg:border-none lg:bg-transparent lg:p-0 lg:shadow-none  dark:lg:bg-transparent
                            ${navOpen ? 
                                "visible block scale-100 opacity-100 lg:flex lg:translate-y-0"
                                : "invisible opacity-0 lg:translate-y-0 lg:scale-100 lg:opacity-100 lg:visible"
                            }
                        `}>
                            ...
                            <div className="mt-12 -ml-1 flex w-full flex-col space-y-2 border-primary/10 dark:border-gray-700 sm:flex-row md:w-max lg:mt-0 lg:mr-6 lg:space-y-0 lg:border-l lg:pl-6">
                                <a href="./pages/contact.html" className="relative ml-auto flex h-9 w-full items-center justify-center before:absolute before:inset-0 before:rounded-full before:bg-primary before:transition-transform before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95 dark:before:border-gray-700 dark:before:bg-primaryLight sm:px-4 lg:before:border lg:before:border-gray-200 lg:before:bg-gray-100 lg:dark:before:bg-gray-800">
